...
app.get("/sse", async (req, res) => {
  console.log("新 SSE 连接请求");
  
  const mcpServer = createMcpServer();
  const transport = new SSEServerTransport("/messages", res);
  
  // 用 transport 对象本身作为 key
  transports.set(transport, mcpServer);
...
app.post("/messages", express.raw({ type: '*/*' }), async (req, res) => {
  ...
  if (entries.length === 0) {
    return res.status(400).json({ error: "没有活跃的 SSE 连接" });
  }
...
app.get("/", (req, res) => {
  res.json({ status: "running", owner: "朝灯", version: "3.1.1", connections: transports.size });
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`记忆库 3.1.1 端口 ${PORT}`));
