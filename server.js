...
app.use((req, res, next) => {
  ...
  res.setHeader("Access-Control-Allow-Headers", "Content-Type, Authorization, X-Requested-With, Accept, Origin, mcp-session-id");
...
function createMcpServer() {
  const server = new McpServer({ name: "朝灯的记忆库", version: "4.3.0" });
...
async function createNewSession() {
  ...
  transport.onclose = () => {
    ...
    if (sid) {
      ...
      console.log(`[MCP] Session 关闭: ${sid}`);
    }
  };
  ...
  if (transport.sessionId) {
    ...
    console.log(`[MCP] 新 Session 建立: ${transport.sessionId}`);
  }

  return transport;
}

// POST: 初始化 session 或发送消息
app.post("/mcp", async (req, res) => {
  console.log("[MCP] 收到 POST 请求");
...
app.get("/mcp", async (req, res) => {
  const sessionId = req.headers["mcp-session-id"];
  console.log(`[MCP] 收到 GET 请求, sessionId: ${sessionId || "无"}`);
  ...
  // 无 session 的 GET：也创建新 session（兼容某些客户端先 GET 的行为）
  console.log("[MCP] 无 session 的 GET，创建新 session");
  const transport = await createNewSession();
  await transport.handleRequest(req, res);
});
...
app.delete("/mcp", (req, res) => {
  ...
  if (sessionId && transports.has(sessionId)) {
    ...
    console.log(`[MCP] Session 删除: ${sessionId}`);
...
// ================== 健康检查 ==================
app.get("/", (req, res) => {
  res.json({ status: "running", owner: "朝灯", version: "4.3.0 (Streamable HTTP)", activeSessions: transports.size });
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`记忆库 4.3.0 (Streamable HTTP) 端口 ${PORT}`));
