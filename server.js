...
function createMcpServer() {
  const server = new McpServer({ name: "朝灯的记忆库", version: "4.2.0" });
...
app.get("/mcp", async (req, res) => {
  const sessionId = req.headers["mcp-session-id"];
  console.log(`[MCP] 收到 GET 请求, sessionId: ${sessionId}`);

  if (sessionId && transports.has(sessionId)) {
    const transport = transports.get(sessionId);
    await transport.handleRequest(req, res);
  } else {
    // 按 Streamable HTTP 规范，无 session 的 GET 返回 400
    res.status(400).json({ 
      error: "Missing or invalid session ID",
      hint: "Send POST first to initialize session" 
    });
  }
});
...
// ================== 健康检查 ==================
app.get("/", (req, res) => {
  res.json({ status: "running", owner: "朝灯", version: "4.2.0 (Streamable HTTP)", activeSessions: transports.size });
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`记忆库 4.2.0 (Streamable HTTP) 端口 ${PORT}`));
