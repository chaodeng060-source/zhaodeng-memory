...
function createMcpServer() {
  const server = new McpServer({ name: "朝灯的记忆库", version: "4.1.0" });
...
app.get("/mcp", (req, res) => {
  console.log("[MCP] 收到 GET 请求 - 返回 405");
...
// ================== 健康检查 ==================
app.get("/", (req, res) => {
  res.json({ status: "running", owner: "朝灯", version: "4.1.0 (Streamable HTTP)", activeSessions: transports.size });
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`记忆库 4.1.0 (Streamable HTTP) 端口 ${PORT}`));
