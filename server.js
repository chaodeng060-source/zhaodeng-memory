...
function createServer() {
    ...
    server.tool("save_memory", "保存记忆到朝灯的记忆库", {
        ...
        const { error } = await supabase.from('memories').insert([{ content, category, importance, created_at: new Date().toISOString() }]);
        return { content: [{ type: "text", text: error ? `失败: ${error.message}` : "已封存。" }] };
    });
    ...
    server.tool("query_memories", "查询朝灯的记忆", {
        ...
        if (error) return { content: [{ type: "text", text: `失败: ${error.message}` }] };
        if (!data?.length) return { content: [{ type: "text", text: "无记忆。" }] };
        return { content: [{ type: "text", text: data.map(m => `[${m.category}] ${m.created_at}\n${m.content}`).join('\n---\n') }] };
    });

    server.tool("delete_memory", "删除记忆", {
        id: z.string().describe("记忆ID")
    }, async ({ id }) => {
        const { error } = await supabase.from('memories').delete().eq('id', id);
        return { content: [{ type: "text", text: error ? `失败: ${error.message}` : "已删除。" }] };
    });
