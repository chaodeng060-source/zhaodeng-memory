...
function createMcpServer() {
  const server = new McpServer({ name: "朝灯的记忆库", version: "3.1.0" });
...
app.get("/sse", async (req, res) => {
  ...
  console.log(`[${sessionId}] 新连接`);
  
  const mcpServer = createMcpServer();
  const transport = new SSEServerTransport("/messages", res);
  connections.set(sessionId, { server: mcpServer, transport });
  
  res.on('close', () => {
    console.log(`[${sessionId}] 断开`);
    connections.delete(sessionId);
  });
  
  try {
    await mcpServer.connect(transport);
    console.log(`[${sessionId}] ✅ 成功`);
  } catch (e) {
    console.error(`[${sessionId}] ❌ 失败:`, e.message);
    connections.delete(sessionId);
  }
});

app.post("/messages", express.raw({ type: '*/*' }), async (req, res) => {
  const conn = Array.from(connections.values()).pop();
  if (!conn) return res.status(400).json({ error: "无连接" });
  try {
...
app.get("/", (req, res) => {
  res.json({ status: "running", owner: "朝灯", version: "3.1.0", connections: connections.size });
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`记忆库 3.1.0 端口 ${PORT}`));
